 ...
 * @internal
 */
interface Method<TInput = unknown, TOutput = unknown> {
	description: string;
	inputSchema: z.ZodObject<any, any, any>;
	outputSchema: z.ZodTypeAny;
	handler: (args: TInput, user?: any) => Promise<TOutput>;
...
export type MethodDefinition<TInput extends z.ZodObject<any, any, any> | z.ZodTypeAny, TOutput> = {
	...
	handler: (args: z.infer<TInput>, user?: any) => Promise<TOutput>;
};

// --- Specialized Method Definitions for Automatic Schema Inference ---

// Helper type to create a partial schema excluding server-managed fields
type CreateInputType<T extends z.ZodObject<any>> = Omit<z.infer<T>, "id" | "createdAt" | "updatedAt">;

// Helper type to create an update input type (partial + required id)
type UpdateInputType<T extends z.ZodObject<any>> = Partial<z.infer<T>> & { id: string };

type CreateMethod<T extends z.ZodObject<any>> = Omit<MethodDefinition<z.ZodObject<any>, z.infer<T>>, "inputSchema" | "outputSchema"> & { 
	handler: (args: CreateInputType<T>, user?: any) => Promise<z.infer<T>> 
};

type UpdateMethod<T extends z.ZodObject<any>> = Omit<MethodDefinition<z.ZodObject<any>, z.infer<T>>, "inputSchema" | "outputSchema"> & { 
	handler: (args: UpdateInputType<T>, user?: any) => Promise<z.infer<T>> 
};
...
export interface ResourceConfig<T extends z.ZodObject<any, any, any>> {
	...
	methods: Record<string, Method<unknown, unknown>>;
}
...
export interface ResourceBlueprint<T extends z.ZodObject<any, any, any>> {
	...
	methods?: {
		get?: { 
			handler: (args: { id: string }, user?: any) => Promise<z.infer<T> | undefined>; 
			description?: string; 
			inputSchema?: z.ZodObject<any>; 
		...
		[customMethodName: string]: MethodDefinition<any, any> | undefined;
	};
}
 ...
 */
function createCreateSchema<T extends z.ZodObject<any, any, any>>(schema: T) {
 ...
 */
function createUpdateSchema<T extends z.ZodObject<any, any, any>>(schema: T) {
	const editableSchema = extractEditableProperties(schema);
...
export function createResource<T extends z.ZodObject<any, any, any>>(blueprint: ResourceBlueprint<T>): ResourceConfig<T> {
	...
	const processedMethods: Record<string, Method<unknown, unknown>> = {};
	...
	if (methodBlueprints) {
		...
		if (methodBlueprints.get?.handler) {
			getHandler = methodBlueprints.get.handler;
			// Don't delete it - we'll process it as a regular method too
		}

		for (const methodName in methodBlueprints) {
			const methodDef = methodBlueprints[methodName as keyof typeof methodBlueprints];
