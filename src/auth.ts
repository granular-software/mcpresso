 ...
 * 3. No auth: Don't provide this config at all
 */
...
export function createMCPAuthMiddleware(authConfig: MCPAuthConfig, serverUrl?: string) {
  ...
  // Validate that we have either issuer or oauth server
  if (!authConfig.issuer && !authConfig.oauth) {
    throw new Error("MCPAuthConfig must provide either 'issuer' (for external OAuth) or 'oauth' (for integrated OAuth)");
  }
